...
function StashPage() {
  const { data, isLoading, isError, error, deleteItem, isDeleting } =
    useStash();
  const [isAddDialogOpen, setIsAddDialogOpen] = useState(false);
  const [withdrawDialogItem, setWithdrawDialogItem] =
    useState<StashItemWithHistory | null>(null);
  ...
  return (
    <Flex direction="column" gap="4" maxWidth="900px" className="mx-auto p-4">
        ...
        <div>
          ...
          {data && (
            <Text size="2" color="gray">
              Gesamt: {data.total_current?.toFixed(1) || 0}g verfügbar |{" "}
              {data.total_withdrawn?.toFixed(1) || 0}g entnommen
            </Text>
          )}
        </div>
      ...
      {data && data.items.length === 0 ? (
        ...
        <Table.Root variant="surface">
          ...
          <Table.Body>
            {data?.items.map((item) => {
              const progress =
                (item.current_amount / item.initial_amount) * 100;
              const progressColor =
                ...
                <Table.Row key={item.id}>
                  ...
                  <Table.Cell>
                    <Flex direction="column" gap="1">
                      <Text weight="bold">
                        {item.current_amount.toFixed(1)}g
                      </Text>
      ...
      {withdrawDialogItem && (
        <WithdrawDialog
          item={withdrawDialogItem}
          isOpen={true}
          onClose={() => setWithdrawDialogItem(null)}
        />
      )}
        ...
        >
          <Dialog.Content style={{ maxWidth: 450 }}>
            ...
            <Dialog.Description size="2" mb="4">
              Möchtest du "{deleteConfirmItem.item_name}" wirklich löschen? Alle
              Entnahme-Daten bleiben erhalten.
            </Dialog.Description>
