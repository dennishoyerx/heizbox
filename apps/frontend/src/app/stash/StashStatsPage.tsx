...
function StashStatsPage() {
	...
	const COLORS = ['var(--blue-9)', 'var(--green-9)', 'var(--amber-9)', 'var(--orange-9)', 'var(--purple-9)']
	...
	const withdrawalTrend =
		data?.daily_withdrawals?.map((day) => ({
			date: day.date,
		...
		<Flex direction='column' gap='4' maxWidth='1200px' className='mx-auto p-4'>
			...
			<div className='grid grid-cols-1 md:grid-cols-3 gap-4'>
				...
				<div className='bg-gradient-to-br from-green-500 to-green-600 p-6 rounded-lg shadow-lg text-white'>
					...
					<p className='text-3xl font-bold mt-2'>{data?.total_current_amount.toFixed(1) || 0}g</p>
				</div>
				...
				<div className='bg-slate-800 p-6 rounded-lg shadow'>
					...
					<ResponsiveContainer width='100%' height={300}>
						<LineChart data={withdrawalTrend}>
							...
							/>
							<Line type='monotone' dataKey='quantity' stroke='#82ca9d' name='Menge (g)' strokeWidth={2} />
						</LineChart>
					</ResponsiveContainer>
				</div>
