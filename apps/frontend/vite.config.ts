...
export default defineConfig(() => ({
  ...
  server: {
    ...
    proxy: {
      ...
      "/ws": {
        target: process.env.VITE_PUBLIC_API_URL || "http://127.0.0.1:8787",
        ws: true,
      },
    },
  },
  ...
  resolve: {
    alias: {
      "@heizbox/utils": fileURLToPath(
        new URL("../../libs/utils/src/index.ts", import.meta.url),
      ),
  ...
  define: {
    ...
    "import.meta.env.VITE_PUBLIC_API_URL": JSON.stringify(
      process.env.VITE_PUBLIC_API_URL || "http://127.0.0.1:8787",
    ),
  },
}));
