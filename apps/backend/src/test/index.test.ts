...
describe('API Routes', () => {
  it('should return heat cycles from the database', async () => {
    const mockDb = {
      prepare: vi.fn(() => ({
        bind: vi.fn(() => ({
  ...
  it('should create a heat cycle', async () => {
    const mockDb = {
      prepare: vi.fn(() => ({
        bind: vi.fn(() => ({
  ...
  it('should create a heat cycle with default cycle when cycle is not provided', async () => {
    const mockDb = {
      prepare: vi.fn(() => ({
        bind: vi.fn(() => ({
          first: vi.fn(() => Promise.resolve({ count: 0 })),
