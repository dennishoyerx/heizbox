...
export class StashService {
	...
	async createStashItem(name: string, initial_amount: number) {
		...
		await this.repository.createItem(id, name.trim(), initial_amount)
	...
	async withdrawFromStash(item_id: string, amount: number) {
		if (amount <= 0) {
		...
		if (item.current_amount < amount) {
			throw new Error(`Insufficient quantity. Available: ${item.current_amount}g, requested: ${amount}g`)
		}
	...
	async getStashStats(range: string) {
		...
		const dailyWithdrawals = await this.repository.getDailyWithdrawalStats(start, now)
