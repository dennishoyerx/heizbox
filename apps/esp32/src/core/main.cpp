...
void setup() {
    ...
    digitalWrite(HardwareConfig::FIRE_BUTTON_PIN, HIGH); // enable pull-up

    // A small delay to allow the user to press the button during startup
    delay(50); 

    if (digitalRead(HardwareConfig::FIRE_BUTTON_PIN) == LOW) {
        // Safe mode for OTA update
        Serial.begin(115200);
