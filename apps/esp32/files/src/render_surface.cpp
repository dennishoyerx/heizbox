...
RenderSurface Screen::createSurface(int16_t w, int16_t h) {
...
void Screen::releaseSurface(RenderSurface& s) {
  if (!s.sprite) return;
  PoolEntry e;
  e.sprite = s.sprite;
  e.w = s.sprite->width();
...
void Screen::withSurface(int16_t w, int16_t h, int16_t targetX, int16_t targetY, SurfaceCallback cb) {
